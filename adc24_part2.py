...
while True:
    ...
    # print(t, candidats)
    next_candidats = set()
    for (x, y, state) in candidats:
        if (x, y) in maps[t % timeloop]:
            continue
        if (x, y, state, t % timeloop) in seen:
            continue
        seen.add((x, y, state, t % timeloop))
        if x == xsize - 1 and y == ysize:
            ...
            if state == 0:
                state = 1
        elif x == 0 and y == -1 and state == 1:
            state = 2
        next_candidats.add((x, y, state))
        for (nx, ny) in [(x + 1, y), (x - 1, y), (x, y + 1), (x, y - 1)]:
            if (nx >= 0 and nx < xsize and (ny >= 0 or (ny == -1 and nx == 0))
                    and (ny < ysize or (ny == ysize and nx == xsize - 1))):
                next_candidats.add((nx, ny, state))
    candidats.clear()
